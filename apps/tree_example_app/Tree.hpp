...
public:
    ...
    Node* findMin( Node t ) const // from book
    {
    if( &t == nullptr )
    return nullptr;
    if( &(t._lft) == nullptr )
    return &t;
    return findMin( *t._lft );
    }
...
private:
    ...
    do_remove( x, *t._rgt );
     else if( t._lft != nullptr && t._rgt != nullptr ) 
    {
    t._val = findMin( *t._rgt )->_val;
    do_remove( t._val, *t._rgt );
    }
    else
    {
    Node oldNode = t;
    t = ( t._lft != nullptr ) ? *(t._lft) : *(t._rgt);
    delete &oldNode;
    }
    }
